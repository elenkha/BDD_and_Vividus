...
Lifecycle:
Examples:
|firstName                  |email                             |boardName                               |placeName                               |
|#{generate(Name.firstName)}|#{generate(Internet.emailAddress)}|#{generate(regexify '[a-z]{5}[A-Z]{2}')}|#{generate(regexify '[a-z]{5}[A-Z]{2}')}|
|#{generate(Name.firstName)}|#{generate(Internet.emailAddress)}|#{generate(regexify '[a-z]{5}[A-Z]{2}')}|#{generate(regexify '[a-z]{5}[A-Z]{2}')}|
...
When I issue a HTTP POST request for a resource with the URL 'https://api.trello.com/1/organizations/?key=44f43bbb1565b45823721eb4ac55c26b&token=89541b6722c9a43eacd5eb5866ee233475e03f01fcff81635abe34ddd997292a&displayName=<placeName>'
